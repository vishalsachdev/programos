...
**Runs on your laptop in an afternoon (Telegram first, email next), graduates to a VPS or cloud when you're ready.** Same spec, same curriculum repo, same audit log — only the channel transports and host runtime change as you scale up. See [`docs/08-deployment-tiers.md`](./docs/08-deployment-tiers.md).
...
| [`docs/08-deployment-tiers.md`](./docs/08-deployment-tiers.md) | Laptop → VPS → Cloud. What changes between tiers (channels, ingress, secrets), what stays constant (curriculum repo, audit log, skill loop). |
...
| [`scripts/init.sh`](./scripts/init.sh) | One-command bootstrap for a new adopter — scaffolds a curriculum repo and seeds it with a brainstorm prompt. |
...
1. **Bootstrap your curriculum repo**:
   ```
   git clone https://github.com/vishalsachdev/programos.git
   ./programos/scripts/init.sh <unit-slug> ~/code/<unit-slug>-curriculum
   ```
